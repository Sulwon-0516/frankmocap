...
def gen_video_out(out_dir, seq_name, fps=25):
    ...
    ffmpeg_cmd = f'ffmpeg -y -f image2 -framerate {str(int(fps))} -pattern_type glob -i "{in_dir}/*.jpg"  -pix_fmt yuv420p -c:v libx264 -x264opts keyint=25:min-keyint=25:scenecut=-1 -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" {out_path}'
